...
class Backend(CSVBackend):
    debug_filename = os.path.join(os.path.dirname(__file__), '../tests/data/aarush_diam.csv')
    infile_glob = os.path.join(settings.FTP_ROOT, 'aarush-diam/*.csv')
    ...
    def write_diamond_row(self, line, blank_columns=None):
        ...
        (
            ...
            unused_has_digital_cert, # Digital Cert
            cut_grade, #  Make (Cut Grade)
            unused_hna, # H&A
            polish, #  Polish
            symmetry, #  Symmetry
            fluorescence, #  Fluorescence Intensity
            unused_rap_price, # Rapaport Price
            unused_off_rap, # % off RAP
            carat_price, #  Price/Ct
            ...
            manmade, # Origin
            unused_memo_status, # Memo Status 
            laser_inscribed, #  Inscription #
            cert_image, # Cert Link
            unused_diamond_image, # Diamond Image
            v360_link, #  Video
            unused_green_dia, # Renewable Energy
        ...
        # comment = cached_clean(comment) or ''
        stock_number = clean(stock_number, upper=True)
        
        manmade = 't' if 'LAB' in manmade else False
        
        if manmade == '' and not include_mined:
        ...
        length, width, depth = split_measurements(dimensions)
