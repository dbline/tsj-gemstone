...
class Backend(CSVBackend):
    debug_filename = os.path.join(os.path.dirname(__file__), '../tests/data/atlantic-diamond.csv')
    infile_glob = os.path.join(settings.FTP_ROOT, 'atlanticdiamond-ftp/1243-diamonds.csv')
    ...
    def get_default_filename(self):
        try:
            ...
            fn = None
        if not fn:
            raise ImportSourceError('No atlantiv_diamond file, aborting import.')
    ...
    def write_diamond_row(self, line, blank_columns=None):
        ...
        (
            ...
            depth_percent,  #Depth %
            table_percent,  #Table %
            girdle_thin,  #Girdle Min
            girdle_thick,  #Girdle Max
            unused_girdle_condition,  #Girdle Condition
            culet,  #Culet Size
            polish,  #polish
            symmetry,  #Symmetry
            fluorescence,  #Fluorescence Intensity
            unused_fluoresccence_color,  #Fluorescence Color
            ...
            unused_diamond_image,  #Diamond Image
            stock_number,  #Vendor Stock Number
            unused_matching_stock_number,  #Pair Vendor Stock Number
            unused_is_matched_pair_separable,  #Is Matched Pair Separable
            ...
            allow_rap_link_feed,  #RapList
            unused_rap_percent,  #Rap%
            unused_shade,  #Shade
            unused_measure_length,  #MeasLength
            unused_measure_width,  #MeasWidth
            unused_measure_depth,  #MeasDepth
            city,  # City
            state,  # State
            country,  # Country
            video_url, #VideoUrl
