...
class Backend(CSVBackend):
    debug_filename = os.path.join(os.path.dirname(__file__), '../tests/data/belgiumdia_inventory.csv')
    infile_glob = os.path.join(settings.FTP_ROOT, 'belgiumwebnet/belgiumdia_*csv')
    ...
    def write_diamond_row(self, line, blank_columns=None):
        ...
        (
            ...
            culet, #  Culet Size
            unused_culet_condition, #  Culet Condition
            unused_graining, #  Graining
            fluorescence, #  Fluorescence Intensity
            fluorescence_color, #  Fluorescence Color
            country, #  Country
            state, #  State / Region
            cert_image, #  Certificate File
            unused_embed_link, #  Image File
            unused_online_report, #  Online Report
            unused_matching_stock_number, #  Matching Pair Stock Ref
            unused_is_pair_separable, #  Pair is Seperable
            unused_price_for_pair, #  Asking Price for Pair
            unused_guaranteed_availability, #  Guaranteed Availability
            comment, #  Remarks
            unused_shade, #  Shade
            unused_milky, #  Milky
