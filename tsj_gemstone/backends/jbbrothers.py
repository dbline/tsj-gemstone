...
class Backend(CSVBackend):
    infile_glob = os.path.join(settings.FTP_ROOT, 'jbbrothers/jbstock.csv') ###
    debug_filename = os.path.join(os.path.dirname(__file__), '../tests/data/jbstock.csv') ###
    ...
    def write_diamond_row(self, line, blank_columns=None): ### The order of the fields here must match the order in the header of the data file
        if blank_columns:
            line = line[:-blank_columns]
        (
            cut,
            stock_number,  #  PktCode
            carat_weight,
            unused_Threads,
            cert_num,
            clarity,
            color,
            fancy_color,  #  NEED
            fancy_color_intensity,  #  NEED
            fancy_color_overtone,  #  NEED
            unused_ColorShade,
            cut_grade,
            fluorescence,
            symmetry,
            ...
            unused_Length,  #  Could USE
            unused_Width,  # Could USE
            depth_percent,
            table_percent,
            girdle_thinnest,
            girdle_thickest,
            unused_CrownAngle,
            unused_CrownHeightPer,
            unused_PavilionHeightPer,
            unused_PavilionAnglePer,
            unused_TotalDepthmm,  # Could USE
            unused_GirdleGIA,
            unused_Ratio,
            unused_StarLength,
            unused_LowerHalf,
