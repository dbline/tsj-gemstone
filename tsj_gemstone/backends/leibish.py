...
class Backend(BaseBackend):
    ...
    def get_default_filename(self):
        files = sorted(glob.glob(INFILE_GLOB), key=os.path.getmtime)
        if len(files):
            fn = files[-1]
        else:
            return
    ...
    def run(self):
        ...
        for line in reader:
            # Rather than fail on malformed CSVs, just empty-pad rows which have fewer columns than the header row
...
def write_diamond_row(line, cut_aliases, color_aliases, clarity_aliases, grading_aliases, fluorescence_aliases, fluorescence_color_aliases, certifier_aliases, markup_list, added_date,
    ...
    length, width, depth = split_measurements('x'.join((length,width,depth)))
